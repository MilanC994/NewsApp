...
const useArticlesContainer = () => {
  ...
  const shouldRenderLoadMoreButton = useMemo(
    () => totalResults > articles.length && articles.length < 100,
  ...
  const moreArticlesLoadingSelector = useMemo(
    () => createLoadingSelector([FETCH_MORE_ARTICLES]),
    []
  )

  const moreArticlesLoading = useSelector((state) => moreArticlesLoadingSelector(state))

  const moreArticlesErrorSelector = useMemo(
    () => createErrorMessageSelector([FETCH_MORE_ARTICLES]),
    []
  )
